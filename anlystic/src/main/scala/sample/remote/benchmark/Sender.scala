...
object Sender {
  def main(args: Array[String]): Unit = {
    ...
    val totalMessages = if (args.length >= 2) args(1).toInt else 50000
    val burstSize = if (args.length >= 3) args(2).toInt else 3
